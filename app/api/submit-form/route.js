...
export async function POST(request) {
  try {
    ...
    if (formData.get('telephone') === 'true') contactPreferences.push('Telephone');
    if (formData.get('sms') === 'true') contactPreferences.push('SMS');
    if (formData.get('emailContact') === 'true') contactPreferences.push('Email');
    if (formData.get('whatsapp') === 'true') contactPreferences.push('WhatsApp');
    ...
    const mailOptions = {
      ...
      to: 'uzairshakeel97531@gmail.com',
      ...
      html: `
        ...
        <p><strong>Contact Preferences:</strong> ${contactPreferences.join(', ') || 'None selected'}</p>
        
        ${imageUrls.length > 0 ? `
          <h2>Uploaded Images:</h2>
          <div>
            ${imageUrls.map(url => `<img src="${url}" alt="Uploaded image" style="max-width: 300px; margin: 10px 0;" />`).join('')}
          </div>
        ` : ''}
      `,
    };
